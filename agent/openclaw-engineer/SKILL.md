...
| Filesystem layout, hidden state files, session/transcript structure, log locations, diagnostic commands, "where is it?" lookup | `references/filesystem-and-state.md` |
...
## Workflow: Debugging a Live OpenClaw Instance

Read `references/filesystem-and-state.md` first — it has the full directory map, hidden state files,
and a "where is it?" lookup table for common symptoms.

**Symptom → First file to check:**
- Agent forgetting things → `openclaw.json` compaction config (softThresholdTokens scaled?) + workspace `memory/` daily notes
- Cron stopped running → `~/.openclaw/cron/jobs.json` → check `enabled` field (silent disable on failure)
- Skill not triggering → `skills/<name>/SKILL.md` frontmatter `description` quality
- Auth/API errors → `~/.openclaw/agents/<id>/agent/auth-profiles.json` + `~/.openclaw/.env`
- Gateway won't start → port conflict: `lsof -i :18789`. Only one gateway per port.
- Channel not responding → `openclaw channels status --probe` + channel config in `openclaw.json`
- Slow responses → bootstrap budget (`/context detail`), skill count (target <15)
- Cost spikes → cron jobs on expensive models, `sessions.json` for `estimatedCostUsd`, thinking level
- Tool permission denied → `~/.openclaw/exec-approvals.json` allowlist + `openclaw sandbox explain`
- Memory search returning garbage → delete `~/.openclaw/memory/<agentId>.sqlite` to force reindex
- Subagent ignoring personality → expected: subagents only get AGENTS.md + TOOLS.md (not SOUL.md)

**After config changes:** hybrid mode (default) hot-applies safe changes. Infrastructure changes require
`openclaw restart` or gateway process restart.
